...
class OctraTool:
        ...
        try:
            with open('2captcha.txt', 'r') as f:
                ...
                if not API_KEY_2CAPTCHA:
                    print("\n❌ Error: File 2captcha.txt kosong")
                    return
    ...
    async def multi_send(self) -> None:
        ...
        try:
            ...
            # Minta input jeda waktu
            try:
                delay_minutes = int(input("\n⏰ Masukkan jeda waktu antar putaran (dalam menit): ").strip())
                if delay_minutes <= 0:
                    print("⚠️ Jeda waktu harus lebih dari 0 menit")
                    return
            except ValueError:
                print("❌ Input tidak valid, menggunakan default 1 menit")
                delay_minutes = 1
            ...
            try:
                with open('wallet.json', 'r') as f:
                    ...
                    if not isinstance(wallets, list) or not all('priv' in w and 'addr' in w for w in wallets):
                        raise ValueError("Format data wallet tidak valid")
                    for wallet in wallets:
                        priv = wallet.get('priv')
                        addr = wallet.get('addr')
                        if not all([priv, addr]):
                            raise ValueError("Data wallet tidak lengkap")
            ...
            # Loop utama untuk pengiriman berulang
            while True:
                print("-" * 40)
                await asyncio.sleep(1)
                ...
                for i, wallet in enumerate(wallets, 1):
                    ...
                    while not wallet_success and wallet_retry_count < max_wallet_retries:
                        ...
                        try:
                            ...
                            # Proses pengiriman ke semua alamat tujuan
                            ...
                            wallet_retry_count += 1
                            if wallet_retry_count < max_wallet_retries:
                                print(f"🔄 Mencoba ulang wallet... ({wallet_retry_count}/{max_wallet_retries})")
                                await asyncio.sleep(2)
                            else:
                                print(f"❌ Gagal setelah {max_wallet_retries} kali percobaan")

                    if wallet_success:
                        print(f"\n✅ Wallet {i} selesai diproses")
                    else:
                        print(f"\n❌ Gagal memproses wallet {i}")
                    
                    await asyncio.sleep(2)

                # Tampilkan ringkasan putaran
                print(f"\n🏁 Putaran selesai!")
                print(f"✅ Berhasil mengirim: {success} transaksi")
                
                # Hitung mundur jeda waktu
                print(f"\n⏳ Menunggu {delay_minutes} menit untuk putaran berikutnya...")
                for remaining in range(delay_minutes * 60, 0, -1):
                    mins, secs = divmod(remaining, 60)
                    print(f"\r⏱️  Akan melanjutkan dalam {mins:02d}:{secs:02d} ", end="")
                    await asyncio.sleep(1)
                print("\n" + "-" * 40)
